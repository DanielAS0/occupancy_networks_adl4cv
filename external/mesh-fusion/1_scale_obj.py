...
class Scale:
    ...
    def run_file(self, filepath):
        ...
        # alternative scaling - don't scale via longest edge but longest distance to origin
        bb_max_translated = bb_max + translation    # centralize bounding box
        norm = np.linalg.norm(bb_max_translated)
        scale_alt = norm / (0.5 - (self.options.padding / 2))
        scales_alt_inv = (
            1/scale_alt, 1/scale_alt, 1/scale_alt
        )

        print("norm of bb_vertices: %f" %norm)

        mesh.translate(translation)
        mesh.scale(scales_alt_inv)
